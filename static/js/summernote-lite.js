...
(function (global, factory) {
  ...
  const Renderer = /** @class */ (function () {
    ...
    Renderer.prototype.render = function ($parent) {
      ...
      if (this.children) {
        const $container_1 = $node.find('.note-children-container');
  ...
  const TooltipUI = /** @class */ (function () {
    function TooltipUI ($node, options) {
      ...
      if (this.options.trigger !== 'manual') {
        const showCallback_1 = this.show.bind(this);
        const hideCallback_1 = this.hide.bind(this);
        const toggleCallback_1 = this.toggle.bind(this);
  ...
  const DropdownUI = /** @class */ (function () {
    ...
    DropdownUI.prototype.show = function () {
      ...
      const targetMarginRight = parseFloat($(this.options.target).css('margin-right'));
  ...
  function posFromPlaceholder (placeholder) {
    const $placeholder = $$1(placeholder);
  ...
  const Context = /** @class */ (function () {
    ...
    Context.prototype._initialize = function () {
      ...
      const buttons = $$1.extend({}, this.options.buttons);
      Object.keys(buttons).forEach(function (key) {
        _this.memo('button.' + key, buttons[key]);
      });
      const modules = $$1.extend({}, this.options.modules, $$1.summernote.plugins || {});
  ...
  $$1.fn.extend({
    /**
       ...
       */
    summernote () {
      const type = $$1.type(lists.head(arguments));
      const isExternalAPICalled = type === 'string';
      const hasInitOptions = type === 'object';
      const options = $$1.extend({}, $$1.summernote.options, hasInitOptions ? lists.head(arguments) : {});
      // Update options
  ...
  /**
     ...
     */
  const WrappedRange = /** @class */ (function () {
    ...
    WrappedRange.prototype.scrollIntoView = function (container) {
      const height = $$1(container).height();
    ...
    WrappedRange.prototype.pasteHTML = function (markup) {
      const contentsContainer = $$1('<div></div>').html(markup)[0];
  ...
  const Style = /** @class */ (function () {
    ...
    Style.prototype.styleNodes = function (rng, options) {
      ...
      if (expandClosestSibling) {
        if (onlyPartialContains) {
          const nodesInRange_1 = rng.nodes();
    ...
    Style.prototype.current = function (rng) {
      const $cont = $$1(!dom.isElement(rng.sc) ? rng.sc.parentNode : rng.sc);
  ...
  const Bullet = /** @class */ (function () {
    ...
    Bullet.prototype.indent = function (editable) {
      ...
      $$1.each(clustereds, function (idx, paras) {
        ...
        if (dom.isLi(head)) {
          const previousList_1 = _this.findList(head.previousSibling);
          if (previousList_1) {
            paras
      ...
      $$1.each(clustereds, function (idx, paras) {
        ...
        if (headList.parentNode.nodeName === 'LI') {
          paras.map(function (para) {
            const newList = _this.findNextSiblings(para);
    ...
    Bullet.prototype.findList = function (node) {
      return node
        ? lists.find(node.children, function (child) { return ['OL', 'UL'].includes(child.nodeName); })
        : null;
    };
    /**
  ...
  */
  ...
  const Table = /** @class */ (function () {
    ...
    Table.prototype.addRow = function (rng, position) {
      const cell = dom.ancestor(rng.commonAncestor(), dom.isCell);
      const currentTr = $$1(cell).closest('tr');
      const trAttributes = this.recoverAttributes(currentTr);
      const html = $$1('<tr' + trAttributes + '></tr>');
      const vTable = new TableResultAction(cell, TableResultAction.where.Row, TableResultAction.requestAction.Add, $$1(currentTr).closest('table')[0]);
        ...
        switch (currentCell.action) {
          ...
          case TableResultAction.resultAction.SumSpanCount:
            if (position === 'top') {
              ...
              if (isTopFromRowSpan) {
                const newTd = $$1('<div></div>').append($$1('<td' + tdAttributes + '>' + dom.blank + '</td>').removeAttr('rowspan')).html();
                html.append(newTd);
                break;
              }
            }
    ...
    Table.prototype.addCol = function (rng, position) {
      const cell = dom.ancestor(rng.commonAncestor(), dom.isCell);
      const row = $$1(cell).closest('tr');
      const rowsGroup = $$1(row).siblings();
      rowsGroup.push(row);
      const vTable = new TableResultAction(cell, TableResultAction.where.Column, TableResultAction.requestAction.Add, $$1(row).closest('table')[0]);
      const actions = vTable.getActionList();
    ...
    Table.prototype.deleteRow = function (rng) {
      const cell = dom.ancestor(rng.commonAncestor(), dom.isCell);
      const row = $$1(cell).closest('tr');
      const cellPos = row.children('td, th').index($$1(cell));
      const rowPos = row[0].rowIndex;
      const vTable = new TableResultAction(cell, TableResultAction.where.Row, TableResultAction.requestAction.Delete, $$1(row).closest('table')[0]);
      const actions = vTable.getActionList();
      for (let actionIndex = 0; actionIndex < actions.length; actionIndex++) {
    ...
    Table.prototype.deleteCol = function (rng) {
      ...
      const row = $$1(cell).closest('tr');
      const cellPos = row.children('td, th').index($$1(cell));
      const vTable = new TableResultAction(cell, TableResultAction.where.Column, TableResultAction.requestAction.Delete, $$1(row).closest('table')[0]);
      const actions = vTable.getActionList();
      for (let actionIndex = 0; actionIndex < actions.length; actionIndex++) {
        if (!actions[actionIndex]) {
          continue;
    ...
    Table.prototype.createTable = function (colCount, rowCount, options) {
      ...
      trHTML = trs.join('');
      const $table = $$1('<table>' + trHTML + '</table>');
  ...
  const Editor = /** @class */ (function () {
    function Editor (context) {
      ...
      this.createLink = this.wrapCommand(function (linkInfo) {
        ...
        if (isTextChanged) {
          rng = rng.deleteContents();
          const anchor = rng.insertNode($$1('<A>' + linkText + '</A>')[0]);
          anchors.push(anchor);
        } else {
      ...
      this.floatMe = this.wrapCommand(function (value) {
        const $target = $$1(_this.restoreTarget());
      ...
      if ($target && $target.length) {
        ...
        if ($target && $target.length) {
          ...
          if (className) {
            ...
            const $parent = $$1([currentRange.sc, currentRange.ec]).closest(tagName);
    ...
    Editor.prototype.getLinkInfo = function () {
      ...
      const $anchor = $$1(lists.head(rng.nodes(dom.isAnchor)));
      const linkInfo = {
        range: rng,
        text: rng.toString(),
  ...
  const Clipboard = /** @class */ (function () {
  ...
  const CodeView = /** @class */ (function () {
    ...
    CodeView.prototype.purify = function (value) {
      if (this.options.codeviewFilter) {
        ...
        if (this.options.codeviewIframeFilter) {
          const whitelist_1 = this.options.codeviewIframeWhitelistSrc.concat(this.options.codeviewIframeWhitelistSrcBase);
          value = value.replace(/(<iframe.*?>.*?(?:<\/iframe>)?)/gi, function (tag) {
            // remove if src attribute is duplicated
            if (/<.+src(?==?('|"|\s)?)[\s\S]+src(?=('|"|\s)?)[^>]*?>/i.test(tag)) {
              return '';
            }
            for (let _i = 0, whitelist_2 = whitelist_1; _i < whitelist_2.length; _i++) {
              const src = whitelist_2[_i];
    ...
    CodeView.prototype.activate = function () {
      ...
      if (env.hasCodeMirror) {
        const cmEditor_1 = CodeMirror.fromTextArea(this.$codable[0], this.options.codemirror);
        // CodeMirror TernServer
        if (this.options.codemirror.tern) {
          const server_1 = new CodeMirror.TernServer(this.options.codemirror.tern);
  ...
  const Statusbar = /** @class */ (function () {
  ...
  const Fullscreen = /** @class */ (function () {
    ...
    Handle.prototype.initialize = function () {
      ...
      this.$handle.on('mousedown', function (event) {
        if (dom.isControlSizing(event.target)) {
          event.preventDefault();
          event.stopPropagation();
          const $target_1 = _this.$handle.find('.note-control-selection').data('target');
          const posStart_1 = $target_1.offset();
          const scrollTop_1 = _this.$document.scrollTop();
          const onMouseMove_1 = function (event) {
  ...
  const AutoLink = /** @class */ (function () {
    ...
    AutoLink.prototype.replace = function () {
      ...
      if (match && (match[1] || match[2])) {
        ...
        const node = $$1('<a />').html(keyword).attr('href', link)[0];
  ...
  const AutoReplace = /** @class */ (function () {
  ...
  const Placeholder = /** @class */ (function () {
  ...
  const Buttons = /** @class */ (function () {
    ...
    Buttons.prototype.isFontDeservedToAdd = function (name) {
      ...
      return (name !== '' && this.isFontInstalled(name) && !genericFamilies.includes(name));
    };
      ...
      return this.ui.buttonGroup({
        ...
        children: [
          this.button({
            ...
            tooltip,
            click (e) {
              const $button = $$1(e.currentTarget);
          ...
          this.ui.dropdown({
            ...
            callback ($dropdown) {
              ...
              $dropdown.find('input[type=color]').each(function (idx, item) {
                $$1(item).change(function () {
                  const $chip = $dropdown.find('#' + $$1(this).data('event')).find('.note-color-btn').first();
            ...
            click (event) {
              event.stopPropagation();
              const $parent = $$1('.' + className);
              const $button = $$1(event.target);
              const eventName = $button.data('event');
              const value = $button.attr('data-value');
              if (eventName === 'openPalette') {
                const $picker = $parent.find('#' + value);
                const $palette = $$1($parent.find('#' + $picker.data('event')).find('.note-color-row')[0]);
                // Shift palette chips
    ...
    Buttons.prototype.addToolbarButtons = function () {
      ...
      const _loop_1 = function (styleIdx, styleLen) {
        const item = this_1.options.styleTags[styleIdx];
      ...
      this.context.memo('button.fontname', function () {
        const styleInfo = _this.context.invoke('editor.currentStyle');
        // Add 'default' fonts into the fontnames array if not exist
        $$1.each(styleInfo['font-family'].split(','), function (idx, fontname) {
          fontname = fontname.trim().replace(/['"]+/g, '');
          if (_this.isFontDeservedToAdd(fontname)) {
            if (!_this.options.fontNames.includes(fontname)) {
    ...
    Buttons.prototype.updateCurrentStyle = function ($container) {
      ...
      if (styleInfo['font-family']) {
        ...
        const fontName_1 = lists.find(fontNames, this.isFontInstalled.bind(this));
        $cont.find('.dropdown-fontname a').each(function (idx, item) {
          const $item = $$1(item);
          // always compare string to avoid creating another func.
          const isChecked = ($item.data('value') + '') === (fontName_1 + '');
          $item.toggleClass('checked', isChecked);
        });
        $cont.find('.note-current-fontname').text(fontName_1).css('font-family', fontName_1);
      }
      if (styleInfo['font-size']) {
        const fontSize_1 = styleInfo['font-size'];
        $cont.find('.dropdown-fontsize a').each(function (idx, item) {
          const $item = $$1(item);
          // always compare with string to avoid creating another func.
          const isChecked = ($item.data('value') + '') === (fontSize_1 + '');
          $item.toggleClass('checked', isChecked);
        });
        $cont.find('.note-current-fontsize').text(fontSize_1);
      }
      if (styleInfo['line-height']) {
        const lineHeight_1 = styleInfo['line-height'];
        $cont.find('.dropdown-line-height li a').each(function (idx, item) {
          // always compare with string to avoid creating another func.
          const isChecked = ($$1(item).data('value') + '') === (lineHeight_1 + '');
    ...
    Buttons.prototype.tableMoveHandler = function (event) {
      const PX_PER_EM = 18;
      const $picker = $$1(event.target.parentNode); // target is mousecatcher
      const $dimensionDisplay = $picker.next();
      const $catcher = $picker.find('.note-dimension-picker-mousecatcher');
      const $highlighted = $picker.find('.note-dimension-picker-highlighted');
      const $unhighlighted = $picker.find('.note-dimension-picker-unhighlighted');
      ...
      // HTML5 with jQuery - e.offsetX is undefined in Firefox
      if (event.offsetX === undefined) {
        const posCatcher = $$1(event.target).offset();
        posOffset = {
          x: event.pageX - posCatcher.left,
          y: event.pageY - posCatcher.top
        };
  ...
  const LinkDialog = /** @class */ (function () {
    ...
    LinkDialog.prototype.showLinkDialog = function (linkInfo) {
      ...
      return $$1.Deferred(function (deferred) {
        ...
        const $openInNewWindow = _this.$dialog
  ...
  const LinkPopover = /** @class */ (function () {
  ...
  const ImageDialog = /** @class */ (function () {
    ...
    ImageDialog.prototype.initialize = function () {
      ...
      if (this.options.maximumImageFileSize) {
        ...
        const readableSize = (this.options.maximumImageFileSize / 1024 ** unit).toFixed(2) * 1 +
                  ' ' + ' KMGTP'[unit] + 'B';
    ...
    ImageDialog.prototype.showImageDialog = function () {
      const _this = this;
      return $$1.Deferred(function (deferred) {
        const $imageInput = _this.$dialog.find('.note-image-input');
        const $imageUrl = _this.$dialog.find('.note-image-url');
        const $imageBtn = _this.$dialog.find('.note-image-btn');
        _this.ui.onDialogShown(_this.$dialog, function () {
          _this.context.triggerEvent('dialog.shown');
   ...
   */
  const ImagePopover = /** @class */ (function () {
  ...
  const TablePopover = /** @class */ (function () {
  ...
  const VideoDialog = /** @class */ (function () {
    ...
    VideoDialog.prototype.showVideoDialog = function (text) {
      const _this = this;
      return $$1.Deferred(function (deferred) {
        const $videoUrl = _this.$dialog.find('.note-video-url');
        const $videoBtn = _this.$dialog.find('.note-video-btn');
        _this.ui.onDialogShown(_this.$dialog, function () {
          _this.context.triggerEvent('dialog.shown');
  ...
  const HelpDialog = /** @class */ (function () {
    ...
    HelpDialog.prototype.createShortcutList = function () {
      ...
      return Object.keys(keyMap).map(function (key) {
        ...
        const $row = $$1('<div><div class="help-list-item"/></div>');
  ...
  const AirPopover = /** @class */ (function () {
  ...
  const HintPopover = /** @class */ (function () {
    ...
    HintPopover.prototype.createItemTemplates = function (hintIdx, items) {
      ...
      return items.map(function (item, idx) {
        const $item = $$1('<div class="note-hint-item"/>');
    ...
    HintPopover.prototype.createGroup = function (idx, keyword) {
      const _this = this;
      const $group = $$1('<div class="note-hint-group note-hint-group-' + idx + '"/>');
    ...
    HintPopover.prototype.handleKeyup = function (e) {
      ...
      if (!lists.contains([key.code.ENTER, key.code.UP, key.code.DOWN], e.keyCode)) {
        ...
        const keyword_1 = wordRange.toString();
